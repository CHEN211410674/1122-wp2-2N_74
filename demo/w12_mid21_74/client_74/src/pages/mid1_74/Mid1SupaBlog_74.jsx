...
const Mid1SupaBlog_74 = () => {
  const [name, setName] = useState('陳宜真');
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        let { data, error } = await supabase.from('blog_74').select('*');
  ...
  const { mutate: deleteBlog } = useMutation({
    mutationFn: async (id) => {
      try {
        const { error } = await supabase.from('blog_74').delete().eq('id', id);
      } catch (error) {
        console.log(error);
      }
    },
  ...
  const { mutate: clearAll } = useMutation({
    mutationFn: async () => {
      try {
        const { error } = await supabase.from('blog_74').delete().gt('id', 0);
      } catch (error) {
        console.log(error);
      }
    },
...
export default Mid1SupaBlog_74;
